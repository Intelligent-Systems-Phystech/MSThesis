...
\begin{document}
	\begin{frontmatter}
		...
		\tnotetext[t1]{This research was supported by RFBR (projects 19-07-01155, 19-07-00875) and Center of Big Data Storage and Analysis NTI (project 13/1251/2018).}
		\author[mipt]{R.V.~Isachenko\corref{cor1}}
		\ead{roman.isachenko@phystech.edu}
		\author[mipt]{V.V.~Strijov}
		\ead{strijov@phystech.edu} 
...
%$ maps the source $\mathbf{x}$ and the target $\mathbf{y}$ lie  maps theta \boldsymbol{\Theta}.$
...
\begin{equation}
(1 - \alpha) \cdot \underbrace{\bz^{\T} \bQ \bz}_{\text{Sim}(\bX)} - \alpha \cdot \underbrace{ \bb^{\T} \bz}_{\text{Rel} (\bX, \bY)}%\bnu)} 
\rightarrow \min_{\substack{\bz \geq \bZero_n \\ \bOne_n^{\T} \bz=1}}.
\label{eq:qpfs_problem}
\end{equation}
Here, the columns of matrix~$\bX$ are the features, and~$\bY$ %\bnu$ 
is the target. 
...
We conducted a computational experiment with the ECoG data from the NeuroTycho project~\citep{shimoda2012decoding}.
...
\bibliography{papers_qpfs_updated}
