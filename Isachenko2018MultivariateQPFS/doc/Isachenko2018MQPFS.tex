...
The feature selection are used to overcome the multicorrelation in feature representation.
...
The subsequent hand position are correlated. 
...
We measure multicorrelation by mean value of miltiple correlation coefficient.
\[
...
For $k = 2$ and $k = 3$ the scores of some targets becomes zero when $\alpha_3$ increases.
...
We show how the described metrics are changing with increasing the size of feature set. Figure~\ref{fig:ecog_3_1_metrics} illustrates the results for $k = 1$. 
...
The situation is changing for predicting several timestamps. In this case the correlation in~$\bY$ matrix is crucial. 
